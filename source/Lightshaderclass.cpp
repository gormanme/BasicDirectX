...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    result = InitializeShader(device, hwnd, L"assets/shaders/Light.hlsl", L"assets/shaders/Light.hlsl");
    if (!result)
    {
        return false;
    }
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	//Setup description of camera dynamic constant buffer, that is in the vertex shader
	...
	DirectX::XMFLOAT4 diffuseColor, DirectX::XMFLOAT3 cameraPosition, DirectX::XMFLOAT4 specularColor, float specularPower)
{
	...
	//Set position of camera constant buffer in vertex shader
	bufferNumber = 1;

	//Set camera constant buffer in vertex shader with updated values
	deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_cameraBuffer);

    //Set shader texture resource in pixel shader
    deviceContext->PSSetShaderResources(0, 1, &texture);

    //Diffuse lighting code:
